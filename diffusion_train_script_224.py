...
def show_images(images, title=""):
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            if idx < len(images):
                plt.imshow(images[idx][0], cmap="cmr.sepia")
                plt.axis('off')
                idx += 1
    fig.suptitle(title, fontsize=30)
    plt.savefig(title,facecolor='white', transparent=False ,dpi = 250)
    title = title + '.pdf'
    plt.savefig(title,facecolor='white', transparent=False ,dpi = 250)
...
#                     galaxy zoo loader                       #
#-------------------------------------------------------------#
im_size = 224 # convert to same size as MNIST
transform  = T.Compose([
        ToTensor(),
...
class MyDDPM(nn.Module):
    def __init__(self, network, n_steps=200, min_beta=10 ** -4, max_beta=0.02, device=None, image_chw=(1, 224, 224)):
...
#------------------------#
# define new images func #
#------------------------#
def generate_new_images(ddpm, n_samples=16, device=None, frames_per_gif=100, gif_name="sampling.gif", c=1, h=224, w=224):
...
class MyUNet(nn.Module):
    def __init__(self, n_steps=1000, time_emb_dim=100):
        ...
        self.down3 = nn.Sequential(
            #nn.Conv2d(40, 40, 2, 1), # uncomment if scale is odd
            nn.SiLU(),
            nn.Conv2d(40, 40, 4, 2, 1) 
        )

        # Bottleneck -- now what do we do here
        ...
        self.up1 = nn.Sequential(
            ...
            nn.SiLU(),
            #nn.ConvTranspose2d(40, 40, 2, 1) # uncomment if scale is odd
        )
...
def training_loop(ddpm, loader, n_epochs, optim, device, display=True, store_path="ddpm_model.pt"):
    ...
    for epoch in tqdm(range(n_epochs), desc=f"Training progress", colour="#00ff00"):
        ...
        if display:
            show_images(generate_new_images(ddpm, device=device), f"Images epoch {epoch + 1}")
...
generated = generate_new_images(
        ...
        n_samples=100,
        device=device,
        gif_name="100_galaxy_sampson.gif"
    )
show_images(generated, "Final galaxies")
